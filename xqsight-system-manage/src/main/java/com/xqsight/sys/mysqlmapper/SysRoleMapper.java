...
public interface SysRoleMapper {
	...
	@Insert("INSERT INTO SYS_ROLE (ROLE_NAME, ROLE_CODE, ROLET_TYPE, ACTIVE, CREATE_TIME, CREATE_OPR_ID, UPDATE_TIME, UPD_OPR_ID, REMARK) VALUES(" 
	+ "#{roleName, jdbcType=VARCHAR},#{roleCode, jdbcType=VARCHAR},#{roleType, jdbcType=NUMERIC},#{active, jdbcType=NUMERIC},#{createTime, jdbcType=TIMESTAMP} ,#{createOprId, jdbcType=VARCHAR} , #{updateTime, jdbcType=TIMESTAMP} , #{updOprId, jdbcType=VARCHAR} , #{remark, jdbcType=VARCHAR})")
	...
	@Select("SELECT * FROM SYS_ROLE WHERE ROLE_NAME LIKE #{roleName, jdbcType=VARCHAR} ORDER BY ROLE_NAME DESC")
	 ...
	 * @throws
	 */
	@Select("SELECT * FROM SYS_ROLE ORDER BY ROLE_NAME ASC")
