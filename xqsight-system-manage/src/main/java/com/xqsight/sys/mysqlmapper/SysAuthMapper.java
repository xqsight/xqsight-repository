...
public interface SysAuthMapper {
	...
	@Select("SELECT SM.* ,CASE WHEN SMR.MENU_ID IS NULL THEN -1 ELSE 0 END ISSELECTED FROM  SYS_MENU SM LEFT JOIN SYS_MENU_ROLE SMR ON SM.MENU_ID = SMR.MENU_ID AND SMR.ROLE_ID=#{roleId, jdbcType=NUMERIC}")
	List<SysMenu> queryAuthMenu(int roleId);
	...
	@Select("SELECT DISTINCT SM.* FROM  SYS_MENU SM ORDER BY SM.SORT ASC")
