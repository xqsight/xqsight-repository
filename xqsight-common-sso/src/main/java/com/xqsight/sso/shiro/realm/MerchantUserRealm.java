...
import com.xqsight.sso.shiro.authc.MerchantUserToken;
 ...
 */
public class MerchantUserRealm extends AuthorizingRealm {
	...
	public boolean supports(AuthenticationToken token) {
		return token == null || token instanceof MerchantUserToken;
	};
	...
	protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken authcToken) throws AuthenticationException {
		...
		return new CustomAuthenticationInfo(user, user.getUserName(), // 用户名
				user.getPassword(), // 密码
				ByteSource.Util.bytes(user.getCredentialsSalt()), // salt=createDate.getTime()+salt
				getName() // realm name
		);
	}
