...
public class FlashFtpUtils {
    ...
    private static Logger logger = LoggerFactory.getLogger(FlashFtpUtils.class);
    ...
    public static Session connectAndLoginSftp(FlashFtpConfig conf) {
        try {
            ...
            sshSession.connect();
            logger.info("Connected SFTP!Host={},Port={},UserName={}", conf.getUserName(), conf.getHost(), conf.getPort());
            return sshSession;
        } catch (Exception e) {
            e.printStackTrace();
        }
     ...
     * @throws IOException
     */
    private static FTPClient connectAndLoginftp(FlashFtpConfig conf) throws IOException {
        FTPClient ftpClient = new FTPClient();
        ftpClient.connect(conf.getHost(), conf.getPort());
        if (ftpClient.isConnected()) {
            ftpClient.login(conf.getUserName(), conf.getUserPwd());
        }
        logger.info("Connected FTP!Host={},Port={},UserName={}", conf.getUserName(), conf.getHost(), conf.getPort());
        return ftpClient;
    }
